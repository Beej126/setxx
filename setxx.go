...
func addToVariable(scope registry.Key, varName, entry string, isUppercase, addToTop bool, beforeEntry, scopeLabel string) error {
	...
	if beforeEntry != "" {
		...
		// Only changed if entry wasn't already at the top
		if !(entryExists && entryIndex == 0) {
			changed = true
		}
	} else {
		// Add to end (default)
		newEntries = append(filteredEntries, entry)
		// Only changed if entry wasn't already at the end
		if !(entryExists && entryIndex == len(entries)-1) {
			changed = true
		}
	}

	if !changed && entryExists {
		fmt.Printf("Leaving as-is in %s:%s", scopeLabel, varName)
		return nil
	}
...
// launchDemoSplitWindow opens a new cmd.exe window to display both user and system versions of the variable
func launchDemoSplitWindow(varName string) {

	psCommand := `echo '';` +
		`echo SYSTEM:; echo ------;` +
		`(Get-ItemProperty -Path 'HKLM:\SYSTEM\CurrentControlSet\Control\Session Manager\Environment').` + varName + ` -split ';';` +
		`echo '';` +
		`echo USER:; echo ------;` +
		`(Get-ItemProperty -Path 'HKCU:\Environment').` + varName + ` -split ';';` +
		`echo '';`

	cmd := exec.Command("cmd.exe", "/c", "start", "", "powershell", "-NoExit", "-Command", psCommand)
	cmd.Run()
}

// main is the entry point for the program
func main() {
	var systemScope, uppercase, addToTop, debug, sure, demo, demoSplit bool
	...
	fs.BoolVar(&demoSplit, "demosplit", false, "Open new cmd window to display both system and user values of the variable")
	...
	// Special case: if only -demosplit and a variable name is given, just show both values and exit
	if demoSplit && !demo && !flagAdd && !flagRemove && len(parsedArgs) == 1 {
		time.Sleep(500 * time.Millisecond)
		launchDemoSplitWindow(parsedArgs[0])
		os.Exit(0)
	}
	...
	if len(parsedArgs) < 2 {
		...
		fmt.Println("    -demosplit = open new cmd window to display both system and user values of the variable")
